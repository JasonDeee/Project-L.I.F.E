...
import { createUserMessage, createAssistantMessage, createSystemMessage } from "../utils/messageUtils";
    ...
    case "COMPLETE_STREAMING":
      return {
        ...
        messages: state.streamingMessage
          ? [...state.messages, { ...state.streamingMessage, content: action.payload }]
          : state.messages,
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleHandshakeResponse = (data: any) => {
      if (data.success && data.chatHistory && Array.isArray(data.chatHistory)) {
        console.log(`📚 Loading ${data.chatHistory.length} messages from server`);
  ...
  useEffect(() => {
    ...
    const handleStreamingChunk = (data: any) => {
      console.log("📨 Received streaming chunk:", data.content?.slice(0, 30) + "...");
      dispatch({
